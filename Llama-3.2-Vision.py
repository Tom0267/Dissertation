...
def analyseVideoFrames(videoId, framePaths, frameWriter):
    ...
    frameLoader = DataLoader(dataset, batch_size=2, shuffle=False, collate_fn=collate, num_workers=2, pin_memory=True, persistent_workers=True) #create DataLoader for batching frames
...
if __name__ == "__main__":
    ...
    with open(RESULTSPATH, "a", newline='', encoding="utf-8", buffering=1) as csvfile, open("frameOutputs.csv", "w", newline='', encoding="utf-8", buffering=1) as framefile:   
        ...
        for videoId, frames in tqdm(groupedFrames.items(), desc="Analysing videos", unit="video"):      #iterate over each video and its frames
            ...
            if not primaryIsViolent and captions:   #if no violence detected, check if captions exist
                combinedText = " ".join(captions)       #combine all captions into a string
                secondPrompt = (
                ...
                with torch.no_grad():       #generate text-only response from model
                    outputIds = model.generate(**inputs, max_new_tokens=50)    #less tokens for text-only response as image description not needed
                ...
                secondaryDecision = "yes" if textClassifier.classify(textOnlyResponse)["label"] == "violent" else "No"  #classify the text-only response
